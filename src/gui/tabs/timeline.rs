// Aba Timeline — por enquanto, o chat da partida renderizado como uma
// linha do tempo com timestamps em mm:ss. Evoluções futuras incluirão
// eventos unificados (kills, upgrades, techs).
//
...
pub fn show(ui: &mut Ui, loaded: &LoadedReplay, config: &AppConfig) {
    ...
    if chat.entries.is_empty() {
        placeholder(
            ui,
            "Nenhuma mensagem de chat. Em breve esta aba exibirá também kills, upgrades e marcos da partida.",
        );
        return;
    }
