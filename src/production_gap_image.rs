...
use crate::production_gap::ProductionGapEntry;
...
const BAR_IDLE: Rgba<u8> = Rgba([230, 160, 30, 255]);
const TICK_COL: Rgba<u8> = Rgba([80, 80, 80, 255]);
const TIME_COL: Rgba<u8> = Rgba([80, 80, 80, 255]);
const IDLE_LABEL_COL: Rgba<u8> = Rgba([255, 255, 255, 255]);

// ── API pública ──────────────────────────────────────────────────────────────

/// Renderiza os gaps de produção de workers como barra de linha do tempo em memória.
///
/// Barra azul-escuro = produção ativa. Trechos amber sobrepostos = ociosidade,
/// com altura proporcional ao percentual de slots ociosos.
pub fn render_production_gap(
    player_number: usize,
    name: &str,
    race: &str,
    mmr: Option<i32>,
    ...
    for entry in entries {
        ...
        // Altura proporcional à ociosidade: idle_slots / capacity
        let idle_ratio = if entry.capacity > 0 {
            entry.idle_slots as f32 / entry.capacity as f32
        } else {
            1.0
        };
        let idle_h = (idle_ratio * BAR_H as f32).round() as u32;
        if idle_h == 0 {
            continue;
        }

        // Pintar amber a partir do topo da barra (ociosidade fica em cima)
        draw_filled_rect_mut(
            &mut img,
            Rect::at(x_start as i32, bar_y as i32).of_size(w, idle_h),
            BAR_IDLE,
        );

        // Rótulo do idle_slots: dentro do trecho se couber, senão acima da barra
        let label = entry.idle_slots.to_string();
        let label_w = text_size(label_scale, &font, &label).0;
        let char_h = FONT_SIZE.ceil() as u32;
        if label_w + 4 <= w && char_h <= idle_h {
            let label_x = x_start as i32 + (w as i32 - label_w as i32) / 2;
            let label_y = bar_y as i32 + (idle_h as i32 - char_h as i32) / 2;
            draw_text_mut(&mut img, IDLE_LABEL_COL, label_x, label_y, label_scale, &font, &label);
        } else {
...
/// Salva os gaps de produção de workers como barra de linha do tempo PNG.
pub fn write_production_gap_png(
    player_number: usize,
    name: &str,
    race: &str,
    mmr: Option<i32>,
    ...
    let img = render_production_gap(
        player_number, name, race, mmr, entries,
        efficiency_pct, game_loops, loops_per_second,
    )?;
