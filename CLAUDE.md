...
- **The overlay templates are composed, not monolithic:** `data/overlay/` ships pages (`index.html`, `stats-dashboard.html`, `live-players.html`) that `{% extends %}` a `base.html` skeleton and `{% include %}` pieces from `partials/`, with styling split into `tokens.css` (palette) → `components.css` (pieces) → per-page layout. The whole set is embedded via `include_str!` in `assets::DEFAULTS`, which is both the bootstrap/restore source *and* the fallback the render loader uses when a file is missing from disk — a deleted piece can never blank an overlay mid-stream. Adding a template file means one entry in `DEFAULTS`; the render tests iterate that table rather than listing files.
